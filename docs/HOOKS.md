...
Документация описывает все кастомные хуки приложения, их назначение и использование.

## 🔄 Основные хуки

### useStats
Универсальный хук для работы со статистикой тренировок.
...
**Опции:**
- `includeIncomplete`: boolean - включать незавершенные тренировки
- `userWeight`: number - вес пользователя для расчетов
...
- `touched` - поля, которые были затронуты
- `isSubmitting` - состояние отправки
- `handleChange` - обработчик изменения
- `handleBlur` - обработчик потери фокуса
- `handleSubmit` - обработчик отправки
- `setValues` - установка значений
- `setErrors` - установка ошибок
...
**Опции:**
- `immediate`: boolean - выполнить сразу
- `onSuccess`: (data) => void - колбэк успеха
...
### useTodayStats
Хук для получения статистики за сегодня.
...
### useNavigationState
Хук для определения состояния навигации.
