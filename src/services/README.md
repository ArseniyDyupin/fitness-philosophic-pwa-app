...
Модуль services содержит всю бизнес-логику приложения, организованную по доменам.
...
- `merge` - объединить с существующими данными
- `skip` - пропустить дубликаты
...
**Алгоритмы расчета:**
- MET (Metabolic Equivalent of Task) для упражнений
- Учет веса пользователя
- Поправки на интенсивность
...
### Структура сервиса
```tsx
// src/services/new/NewService.ts
export class NewService {
  ...
  async performAction(params: ActionParams): Promise<ActionResult> {
    try {
      ...
      // Логирование
      this.logAction('performAction', params, result)
  ...
  private logAction(method: string, params: any, result: any): void {
    // Логирование
  }

  private handleError(error: Error, params: any): void {
    // Обработка ошибок
  }
}
...
```tsx
// src/services/new/index.ts
export { NewService, newService } from './NewService'
export type { NewServiceConfig, ActionParams, ActionResult } from './NewService'
